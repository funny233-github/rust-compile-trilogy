...
#note[
  *题目位置*：`exercises/src/ch04_control.rs`

  *任务*：实现 `compile_if_else` 函数，把 if-else 结构翻译为带标签和条件跳转的 TAC。

  给你：`if (x) { y = 1 } else { y = -1 }`
  ...
  提示：`IfGoto` 在 cond==0 时跳转（和 C 的 `if(!cond) goto` 一样）。if-then 只需一条 `IfGoto` 跳到 end；if-else 需要 `IfGoto` 跳到 else + then 结束后的无条件 `Jump`。

  *验证*：`cd exercises && cargo test ch04`

  *答案*：`exercises/answers/ch04_control.rs`
]
