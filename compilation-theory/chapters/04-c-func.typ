...
#note[
  *题目位置*：`exercises/src/ch04_func.rs`

  *任务*：实现 `stack_offsets` 函数，计算局部变量在栈帧中的偏移量。

  给你：`vec![("a", 4), ("b", 8)]`（名称 + 字节大小）

  你要返回：`vec![("a", -4), ("b", -12)]`（rbp 相对偏移，负值向下增长）

  提示：从偏移 0 开始，每遇到一个变量就减去其大小。`a` 占 4 字节 → -4；`b` 紧接着放在 a 下方 → -4-8=-12。不需要考虑对齐。

  *验证*：`cd exercises && cargo test ch04`

  *答案*：`exercises/answers/ch04_func.rs`
]
