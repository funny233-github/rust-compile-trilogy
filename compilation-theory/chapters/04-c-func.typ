...
#note[
  ...
  *任务*：实现 `stack_offsets` 函数，根据局部变量的大小计算它们在栈帧中的偏移量（rbp 相对偏移，负值向下增长）。

  *验证*：`cd exercises && cargo test ch04`

  *答案参考*：`exercises/answers/ch04_func.rs`
]

提示：从偏移 0 开始，每遇到一个变量就减去其字节大小，结果就是该变量的栈偏移。不需要考虑对齐——这不影响对栈帧核心概念的理解。
