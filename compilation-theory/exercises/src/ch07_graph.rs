...
mod tests {
    use super::*;
    fn set(s: &[&str]) -> HashSet<String> { s.iter().map(|x| x.to_string()).collect() }
    ...
    fn test_chain() {
        let intervals = vec![
            LiveInterval { var: "a".into(), start: 0, end: 3 },
            LiveInterval { var: "b".into(), start: 2, end: 5 },
            LiveInterval { var: "c".into(), start: 4, end: 6 },
        ];
        let g = build_interference_graph(&intervals);
        assert_eq!(g["a"], set(&["b"]));
